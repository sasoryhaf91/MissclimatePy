...
def _plt():
    """
    Lazy import de matplotlib.pyplot para evitar requerirlo durante la
    importación del paquete (útil para entornos headless o test).
    """
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install 'missclimatepy[dev]' or pip install matplotlib"
        ) from e

def plot_inclusion_aggregate(
    ...
    if metric not in sweep_df.columns:
        raise ValueError(f"'{metric}' not in sweep_df")
...
def plot_inclusion_curves(
    ...
    plt.xlabel("include_pct")
    plt.title(f"Station {station} {title_extra}")
    plt.legend()
    plt.tight_layout()
    if out_png:
        _ensure_dir(out_png)
